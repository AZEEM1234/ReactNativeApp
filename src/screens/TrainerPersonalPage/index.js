    ...
    if (type === "Trainer"){
      AsyncStorage.getItem("@getUserData:key", (error, getUserData) => {
        ...
        this.setState({userData:get_user},()=>{
          ...
          this.getTrainerDetails(this.state.userData.id);
...
selectPhotoTapped() {
  ...
  ImagePicker.showImagePicker(options, (response) => {
    ...
    if (response.didCancel) {
      ...
      var string = {"action":"trainer_profilepic","trainer_id":this.state.userData.id,"data":base64};
...
onAddCustomSpeciality(){
  this.setState({addCustomeSpec:!this.state.addCustomeSpec});
}
onChangeText = (text) => {
      ...
      if (response) {
        if (response.status) {
          if (response.data.secondary_images.length > 0) {
            response.data.secondary_images.map((res,i)=>{
              var record = {type: "Image", source:{"uri": res.secondary_image}, id:res.image_primary_id};
              this.state.media[i] = record;
            })
        ...
        Alert.alert("HomeFit","Can't get data now.")
      }
  })
}
onImageUpload(){
...
onAddSpeciality(){
  if (this.state.speciality === "" || this.state.speciality === null){
    Alert.alert("You Should Enter your speciality name", "")
  } else {
      this.setState({loading: true})
      var AddSpecailityDetails = {
        Id : this.state.userData.id,
        SpeacilityName: this.state.speciality
      }
      API.addSpeciality(AddSpecailityDetails).then(async (response) => {
        if (response.status){
          Alert.alert(response.message,"",
            [
              {text: "OK", onPress: this.onAddSpecialityOk.bind(this)}
            ],
            { cancelable: false }
          )
        } else {
...
onAddSpecialityOk(){
  this.setState({loading:false,addCustomeSpec: false,speciality:""})
}
render() {
  return (
    <Container style={styles.container}>
      ...
      <Content padder>
            ...
            <View>
              ...
              <View style={styles.textInputMainView}>
                ...
                <View style={styles.addIconView}>
                  <TouchableOpacity onPress={this.onAddSpeciality.bind(this)}>
        ...
        <View style={styles.hostLiveGrpSessionTextContainer}>
          <Text style={{fontSize:window.width / 20, textAlign:"center"}}>(Host a free 15 minute session to attract more clients nationally.)</Text>
