...
class ViewTrainer extends Component {
  ...
  onDaySelect = day => {
    ...
    AsyncStorage.getItem("@getUserData:key", (err, getUserData) => {
      ...
      if (showUserData === null){
        ...
        if (this.state.bookedappointmntes.length > 0 ){
          var times = []
          this.state.bookedappointmntes.map((res, i) => {
            if (res.appt_date === _selectedDay) {
              times.push({"date":_selectedDay,"time":res.appt_time});
            }
          })
  ...
  onShowModalContinue(){
  this.setState({showModal: false},()=>{
    this.props.navigation.navigate("Payment",{keyVTrainer:"keyVTrainer",getSelectedData:this.state.getSelectedTimedata,trainersList:this.state.trainersList,keyValue:this.state.keyValue});
   });
  }
  getTrainersData(user){
    var Id = this.state.trainersList.id;
  ...
  getParticularAppointments(userId){
    var primaryId = {
      userId: userId,
      trainerId: this.state.trainerData.id
    }
   API.getParticularAppointments(primaryId).then(async (response) => {
      ...
      if (response.status === true) {
        this.setState({bookedappointmntes : response.data,availableslotsspinner:false})
        this.forSelectedGreenArray(response.data);
      } else {
        this.setState({noslotsText: response.message,availableslotsspinner:false})
      }
    }).catch((error)=>{
      this.setState({availableslotsspinner:false})
      Alert.alert("Error",error);
    });
  }
  ...
  render() {
    //alert(JSON.stringify(this.state.trainerData))
