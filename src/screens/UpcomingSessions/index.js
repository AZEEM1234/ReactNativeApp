      ...
      if (response){
        if (response.status === true){
          response.data.map((res,i)=>{
            var date = res.appt_date
            var newdate = moment(date).format("YYYY-MM-DD");
            var time =  res.appt_time
            var data = time.split("-");
            ...
            console.log("sdkl;kdsdlslksdlsk",appointmnet_date)
            res["Dateformat"] = appointmnet_date
          })
          console.log("repsonsedata....",response.data)
       
          var sortedArray = response.data.sort(function(a, b){
            var dateA = new Date(a.Dateformat), dateB = new Date(b.Dateformat);
            return dateA - dateB;
         });
          this.setState({
    ...
    API.getUpcomingSessions(user_id).then(async (response) => {
      if (response){
        if (response.status === true){
          ...
          this.setState({sessions:sortedArray,emptyData:false,spinner:false});
        } else {
