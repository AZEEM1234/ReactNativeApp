...
class ProductDetails extends React.Component {
  ...
  componentWillMount(){
    ...
    this.setState({badgeTotal: this.props.cartTotal.addToCartItem.total});
    for (var i = 1; i < 100; i++){
...
onAddToCardPressed(){
  ...
  alert("Please select Flavour")
    ...
    dataToModal = {};
    this.setState({modalAddtoCart: true});
    var quantity = this.state.qty;
    console.log("onaddtocart",quantity)
    var item = {
...
onModalSizeBack(){
  this.setState({modalSizeVisible: false},()=>{
    responseDuplicateArray = []
  });
}
...
onModalSizeClose(item){
  this.state.finalResponseArray.map((obj,index) => {
    if (obj.size === item.size) {
      this.setState({flavoursArray: item.flavoursList},() => {
        console.log("abcdd: ", this.state.flavoursArray)
        this.setState({modalSizeVisible: false,modalFlavourVisible: true},()=>{
          responseDuplicateArray = []
        });
      });
    }
...
getItem(item){
  ...
  this.setState({selectedSize: item.size,size_id:item.id});
}
getFlavourItem(item){
  console.log("selectedFlavour:  ",JSON.stringify(item));
  this.setState({selectedFlavour: item.flavour_name,flavour_id:item.flavour_id,price:item.price});
}
getNewResponse(){
  ...
  this.setState({finalResponseArray: finalResponseArray});
}

onPicker(){
  Picker.init({pickerData: this.state.quantityList,selectedValue: [1],onPickerConfirm: data => {
    console.log("sdjks;jds;jd",data[0])
    this.setState({qty:data[0]});
...
getFeaturedProducts(){
  ...
  renderData = ({item,index}) => {
    return (
      <TouchableOpacity  style={styles.storeProducts} onPress = {this.onFeaturedProductsTouched.bind(this,item)}>
  ...
  onFeaturedProductsTouched(item){
    this.setState({loading: true})
    API.getSizesFlavours(item.id).then(async (response) => {
      if (response){
          if ( response.status === true ){
          this.setState({productDetails: response.data},()=>{
            this.setState({
        ...
        this.setState({loading: false},()=>{alert("Error getting product details || Check Network");});
      }
    }).catch((error)=>{
      this.setState({loading:false});
      console.log("Console Error",error);
        ...
        <View style={styles.content}>
          ...
          <View style={styles.modalPickersView}>
            <View style={styles.pickerView}>
              <TouchableOpacity style={styles.pickerStyle} onPress={() => {this.setModalVisible(true)}}>
            ...
            <View style={styles.pickerView}>
              <TouchableOpacity style={styles.pickerStyle} onPress={() => {this.setModalVisibleFlavour(true)}}>
                <View style={styles.pickerTextView}>
          ...
          <View style={styles.reviewsHeadingBlock}>
            <Text style={styles.reviewsHeading}>{"Related Products".toUpperCase()}</Text>
          </View>
          <View>
            <FlatList
